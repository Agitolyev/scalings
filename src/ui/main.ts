...
class App {
  ...
  private static readonly RUN_COLORS = [
    'rgba(132, 204, 22, 0.9)',
    'rgba(251, 191, 36, 0.9)',
    'rgba(56, 189, 248, 0.9)',
  ...
  private toggleDashboard(showDashboard: boolean): void {
    const summaryPanel = document.getElementById('summary-panel');
    const dashboard = document.getElementById('simulation-dashboard');
    if (summaryPanel) summaryPanel.classList.toggle('hidden', showDashboard);
  ...
  private clearDashboard(): void {
    ...
    if (count) count.textContent = '0 runs';
  }
  ...
  private renderDashboard(): void {
    ...
    this.recordedRuns.forEach((run, i) => {
      ...
      if (isLatest) tr.className = 'run-latest';
      ...
      if (s.total_dropped > 0) dangerIndices.add(2);       // Dropped
      if (s.drop_rate_percent > 1) dangerIndices.add(3);    // Drop Rate

      metrics.forEach((value, mi) => {
        const td = document.createElement('td');
        td.textContent = value;
        if (dangerIndices.has(mi)) td.className = 'cell-danger';
        tr.appendChild(td);
      });
