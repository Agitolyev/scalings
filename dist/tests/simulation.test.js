...
describe('SimulationService — proportional scaling', () => {
    it('scales proportionally to traffic instead of racing to max_replicas', async () => {
        ...
        assert.ok(result.summary.peak_pod_count <= 8, `peak pods (${result.summary.peak_pod_count}) should be proportional to traffic, not overshoot to max (50)`);
    });
    it('counts starting pods toward desired total', async () => {
        ...
        assert.ok(result.summary.peak_pod_count <= 10, `peak pods (${result.summary.peak_pod_count}) should reflect proportional need, not additive step accumulation`);
    });
});
// ---------------------------------------------------------------------------
// Cost estimation
